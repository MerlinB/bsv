...
Base58Check.decode = async function (s) {
  ...
  var hash = await sha256sha256(data)
...
Base58Check.checksum = async function (buffer) {
  var buf = await sha256sha256(buffer)
  return buf.slice(0, 4)
}

Base58Check.encode = async function (buf) {
  if (!Buffer.isBuffer(buf)) { throw new Error('Input must be a buffer') }
  var checkedBuf = Buffer.alloc(buf.length + 4)
  var hash = await Base58Check.checksum(buf)
  buf.copy(checkedBuf)
