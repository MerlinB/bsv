 ...
 */
Script.buildPublicKeyHashOut = async function (to) {
  $.checkArgument(!_.isUndefined(to))
  $.checkArgument(to instanceof PublicKey || to instanceof Address || _.isString(to))
  if (to instanceof PublicKey) {
    to = to.toAddress()
  } else if (_.isString(to)) {
    to = await new Address(to).initialized
  }
  var s = new Script()
  s.add(Opcode.OP_DUP)
 ...
 */
Script.buildScriptHashOut = async function (script) {
  $.checkArgument(script instanceof Script ||
    (script instanceof Address && script.isPayToScriptHash()))
  var s = new Script()
  s.add(Opcode.OP_HASH160)
    .add(script instanceof Address ? script.hashBuffer : (await Hash.sha256ripemd160(script.toBuffer())))
    .add(Opcode.OP_EQUAL)
 ...
 */
Script.prototype.toScriptHashOut = async function () {
 ...
 */
Script.fromAddress = async function (address) {
 ...
 * @private
 */
Script.prototype._getInputAddressInfo = async function () {
  var info = {}
  if (this.isPublicKeyHashIn()) {
    // hash the publickey found in the scriptSig
    info.hashBuffer = await Hash.sha256ripemd160(this.chunks[1].buf)
    info.type = Address.PayToPublicKeyHash
  } else if (this.isScriptHashIn()) {
    // hash the redeemscript found at the end of the scriptSig
    info.hashBuffer = await Hash.sha256ripemd160(this.chunks[this.chunks.length - 1].buf)
    info.type = Address.PayToScriptHash
  } else {
    return false
  }
 ...
 */
Script.prototype.toAddress = async function (network) {
  var info = this.getAddressInfo()
  if (!info) {
    return false
  }
  info.network = Networks.get(network) || this._network || Networks.defaultNetwork
  return new Address(info).initialized
}
