...
function MultiSigScriptHashInput (input, pubkeys, threshold, signatures) {
  this.instancePromise = (async (instance) => {
    Input.apply(instance, arguments)
    var self = instance
    pubkeys = pubkeys || input.publicKeys
    threshold = threshold || input.threshold
    signatures = signatures || input.signatures
    instance.publicKeys = pubkeys.map(k => k.toString('hex')).sort().map(k => new PublicKey(k))
    instance.redeemScript = Script.buildMultisigOut(instance.publicKeys, threshold)
    $.checkState(await Script.buildScriptHashOut(instance.redeemScript).equals(instance.output.script),
      'Provided public keys don\'t hash to the provided output')
    instance.publicKeyIndex = {}
    _.each(instance.publicKeys, function (publicKey, index) {
      self.publicKeyIndex[publicKey.toString()] = index
    })
    instance.threshold = threshold
    // Empty array of signatures
    instance.signatures = signatures ? instance._deserializeSignatures(signatures) : new Array(instance.publicKeys.length)
  })(this)
