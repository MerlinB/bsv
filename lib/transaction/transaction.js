...
function Transaction (serialized) {
  ...
  this.instancePromise = (async function(instance) {
    await instance._setHash()
    return instance
  })(this)
...
ioProperty.get = function () {
  if (!this.instance) throw new Error('Uninitialized Transaction instance!')
 ...
 * @return {Buffer}
 */
Transaction.prototype._getHash = async function () {
...
Transaction.prototype._setHash = async function() {
  const hash = await this._getHash();
  this._hash = new BufferReader(hash).readReverse().toString('hex')
}
...
Transaction.prototype._checkConsistency = function (arg) {
  ...
  if (arg && arg.hash) {
    console.log("ARG HASH " + arg.hash + " THIS HASH " + this.hash);
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.to = async function (address, amount) {
  //if (!this.instance) throw new Error('Uninitialized Transaction instance!')
  ...
  this.addOutput(new Output({
    script: Script(await new Address(address).initialized),
    satoshis: amount
  }))
  return this
}
...
Transaction.prototype.getSignatures = async function (privKey, sigtype) {
  if (!this.instance) throw new Error('Uninitialized Transaction instance!')
  ...
  var results = []
  var hashData = await Hash.sha256ripemd160(privKey.publicKey.toBuffer())
