...
Message.sign = async function (message, privateKey) {
  return new Message(message).sign(privateKey)
}

Message.verify = async function (message, address, signature) {
  return new Message(message).verify(address, signature)
}

Message.MAGIC_BYTES = Buffer.from('Bitcoin Signed Message:\n')

Message.prototype.magicHash = async function magicHash () {
 ...
 */
Message.prototype.sign = async function sign (privateKey) {
  var signature = await this._sign(privateKey)
  return signature.toCompact().toString('base64')
}

Message.prototype._verify = async function _verify (publicKey, signature) {
 ...
 * @returns {Boolean}
 */
Message.prototype.verify = async function verify (bitcoinAddress, signatureString) {
  $.checkArgument(bitcoinAddress)
  $.checkArgument(signatureString && _.isString(signatureString))

  if (_.isString(bitcoinAddress)) {
    bitcoinAddress = await Address.fromString(bitcoinAddress)
  }
  var signature = Signature.fromCompact(Buffer.from(signatureString, 'base64'))

  // recover the public key
  var ecdsa = new ECDSA()
  ecdsa.hashbuf = await this.magicHash()
  ecdsa.sig = signature
  var publicKey = ecdsa.toPublicKey()

  var signatureAddress = await Address.fromPublicKey(publicKey, bitcoinAddress.network)

  // check that the recovered address and specified address match
  if ((await bitcoinAddress.toString()) !== (await signatureAddress.toString())) {
